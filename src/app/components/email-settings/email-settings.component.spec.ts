...
describe('EmailSettingsComponent', () => {
  ...
  describe('loadHourlyMemorizationReminderTemplate', () => {
    it('loads memorization template key from admin_settings', async () => {
      mockSupabaseService.client.from = vi.fn(() => ({
        select: vi.fn((cols: string) => ({
          eq: vi.fn(() => ({
            maybeSingle: vi.fn(() =>
              String(cols).includes('user_hourly_memorization_reminder_template_key')
                ? Promise.resolve({
                    data: {
      ...
      await component.loadHourlyMemorizationReminderTemplate();

      expect(component.userHourlyMemorizationReminderTemplateKey).toBe(
        'user_hourly_memorization_reminder_with_spotlight'
      );
      expect(component.loadingHourlyMemorizationTemplate).toBe(false);
    });
  });

  describe('saveHourlyMemorizationReminderTemplate', () => {
    it('updates admin_settings and shows success', async () => {
      const eqMock = vi.fn().mockResolvedValue({ error: null });
      const updateMock = vi.fn().mockReturnValue({ eq: eqMock });
      mockSupabaseService.client.from = vi.fn(() => ({
      ...
      component.userHourlyMemorizationReminderTemplateKey =
        'user_hourly_memorization_reminder_with_spotlight';
      const emitSpy = vi.spyOn(component.onSave, 'emit');

      await component.saveHourlyMemorizationReminderTemplate();
      ...
      expect(eqMock).toHaveBeenCalledWith('id', 1);
      expect(component.successHourlyMemorizationTemplate).toBe(true);
      expect(mockToastService.success).toHaveBeenCalledWith(
        'Hourly memorization reminder template saved.'
      );
      expect(emitSpy).toHaveBeenCalled();
      expect(component.savingHourlyMemorizationTemplate).toBe(false);
    });
  });
