...
- **Readability (booklet)**: Half-letter panels use **generous** inset (**~0.75in** bottom) and **13px** body. **`estimateBookletUnitWeight`** now scales compact **Updates** by **full** first-update Markdown (narrow-column wrap premium + **~1.48×** chars), replaces the old flat **~380** reserve, and **`getBookletDescriptionSegmentMaxChars`** shortens description splits when updates are heavy. **`BOOKLET_PANEL_BOTTOM_SLACK`** **~620**, **max five** stacked cards per column as a **heuristic** for the static HTML fallback. **`(continued)`** — see **`splitBookletMarkdownIntoPanelParts`**, **`packBookletUnitsIntoPageChunks`** in [`print.service.ts`](src/app/services/print.service.ts).
- **Measured packing (booklet)**: When the booklet HTML opens in a browser, an inline script ([`booklet-measure-inline.ts`](src/app/lib/booklet-measure-inline.ts)) **measures** each prayer fragment in a hidden panel and **packs** reader pages until content would exceed the panel bottom, then **re-imposes** saddle-stitch surfaces so more short cards fit per half-letter page than the weight-only estimate alone. The string still includes heuristic chunks for first paint / non-JS test environments.
