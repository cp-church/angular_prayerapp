...
- **Data**: Migration [`20260721120000_memorization_recite_mode.sql`](../supabase/migrations/20260721120000_memorization_recite_mode.sql) — `admin_settings.memorization_recite_enabled`, `memorization_recite_usage` ledger, `is_admin()` aligned with `email_subscribers`, and secured admin RPC `get_memorization_recite_usage_summary` (JWT or recent `admin_login` MFA verification).
