...
- **Readability (booklet)**: Half-letter panels use **generous** side/top/bottom inset (**~0.75in** bottom) and **13px** body copy with **roomier** prayer cards (**padding**, **11px** gaps between cards, **13.5px** “For” row). Packing uses **`estimateBookletUnitWeight`** (Markdown **× ~1.25**, newline/list premiums, frame **~228**, update reserve) against **`BOOKLET_PANEL_PACK_BUDGET` ~3400** minus **`h2`** (~**275**) and bottom slack (~**460**) so larger type still tends to split chunks before **`overflow:hidden`**. **`BOOKLET_MARKDOWN_CHARS_PER_PANEL` ~1750** and **up to seven** stacked cards per column when weights allow; long bodies use **`(continued)`**. Short single-segment descriptions use **`trimEnd()`** so trailing whitespace matches split segments. **`Prayer cards`** match **Print Prayers**. Booklet covers unchanged aside from panel math here.
