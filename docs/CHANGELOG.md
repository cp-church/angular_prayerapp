...
- **UX**: Recite mode button now appears for all enabled memorization items (not only single-verse references). Choosing Recite on a multi-verse passage shows an inline warning in the mode picker that transcription only supports single verses.
