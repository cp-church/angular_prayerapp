...
- **Behavior**: [`send-user-hourly-prayer-reminders`](supabase/functions/send-user-hourly-prayer-reminders/index.ts) fills **`{{spotlightPrayerRequester}}`** from community prayer **`prayers.requester`** (who submitted). Personal spotlight picks leave it **empty** (`personal_prayers` has no requester column). Template substitution allows optional spaces around names (e.g. `{{ spotlightPrayerRequester }}`).
