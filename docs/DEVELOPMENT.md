...
- **Edge function**: `supabase/functions/send-user-hourly-prayer-reminders/` — same auth model as **`send-prayer-reminders`**: Supabase Edge **JWT verification** on `Authorization` / `apikey`; no extra in-function check. Uses `APP_URL` for email/push links. Prefers **push** when `email_subscribers.receive_push` is true and a `device_tokens` row exists; otherwise **email** via `send-email`.
