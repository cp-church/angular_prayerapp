...
- **Edge function**: `supabase/functions/send-user-hourly-prayer-reminders/` — authorizes if **`Authorization: Bearer` equals the function’s `SUPABASE_SERVICE_ROLE_KEY`** (trimmed) **or** if header **`X-Hourly-Prayer-Reminders-Secret`** equals secret **`HOURLY_PRAYER_REMINDERS_SECRET`** (`supabase secrets set HOURLY_PRAYER_REMINDERS_SECRET=…`). The second path avoids 401 when GitHub stores a different key shape than the Edge runtime (e.g. new `sb_secret_…` vs legacy JWT). Uses `APP_URL` for email/push links. Prefers **push** when `email_subscribers.receive_push` is true and a `device_tokens` row exists; otherwise **email** via `send-email`.
- **GitHub Actions**: `.github/workflows/send-user-hourly-prayer-reminders.yml` (`cron: 0 * * * *`). Set repo secret **`HOURLY_PRAYER_REMINDERS_SECRET`** to the same value as the function secret, and keep **`SUPABASE_SERVICE_KEY`** as a valid project JWT for the gateway (**service_role** or **anon**). If you use **anon** for Bearer/apikey, rely on the `X-Hourly-Prayer-Reminders-Secret` check for authorization.
